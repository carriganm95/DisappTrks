...
CandidateTrackProducer::CandidateTrackProducer (const edm::ParameterSet& iConfig) :
  ...
  isolatedTracksTag_ (iConfig.getParameter<edm::InputTag> ("isolatedTracksTag")),
  ...
  use_dEdx_         (iConfig.getParameter<bool> ("usedEdx"))
{
  ...
  lostTracksToken_      = consumes<pat::PackedCandidateCollection> (lostTracksTag_);
  pfCandidatesToken_    = consumes<pat::PackedCandidateCollection> (pfCandidatesTag_);
...
CandidateTrackProducer::filter (edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  iEvent.getByToken (isolatedTracksToken_, tracks);
  ...
  for (const auto &track : *tracks) {
    ...
    //TODO set dedx based on the dedx saved in iso trks
    //Curently turn off use_dEdx
